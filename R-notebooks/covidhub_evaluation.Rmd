...
for (i in 1:length(forecasters)) {
  forecast_dates[[i]] = tryCatch({
    as.Date(get_forecast_dates(forecasters[i]))
  },
...
if (exists("predictions_cards")){
  ...
  for (i in 1:length(predictions_cards)){
    forecaster_predictions_cards = predictions_cards[[i]]
    pred_dates = lapply(forecaster_predictions_cards, 
                        FUN = function(x) attributes(x)$forecast_date)
    seen_dates[[i]] = unique(do.call("c", pred_dates))
    name_of_forecaster = attributes(
                          forecaster_predictions_cards[[1]])$name_of_forecaster
    names(seen_dates)[[i]] = name_of_forecaster
  }
} else {
  for (i in 1:length(forecasters)){
    seen_dates[[i]] = list()
  }
  names(seen_dates) = forecasters
}
```
...
for (i in 1:length(forecasters)) {
  ...
  # previous day
  extra_dates = comparable_forecast_dates[(comparable_forecast_dates + 1) %in% comparable_forecast_dates]
  comparable_forecast_dates = as_date(setdiff(comparable_forecast_dates, extra_dates))
  if(forecasters[[i]] %in% names(seen_dates)){
    forecaster_idx = which(names(seen_dates) == forecasters[[i]])
    comparable_forecast_dates = as_date(setdiff(comparable_forecast_dates, seen_dates[[forecaster_idx]]))
  }
...
```{r}
predictions_cards_new = vector("list", length = length(forecasters))
for (i in 1:length(forecasters)) {
  cat(forecasters[i], "...\n")
  predictions_cards_new[[i]] = tryCatch({
...
old_forecaster_idxs = match(forecasters, names(seen_dates))

# Combine old and new predictions cards
predictions_cards_comb = vector("list", length = length(forecasters))
for (i in 1:length(forecasters)){
  if (is.na(old_forecaster_idxs[i])){
    #as.list needed to handle possible NULL values
    predictions_cards_comb[[i]] = as.list(predictions_cards_new[[i]])
  } else {
    predictions_cards_comb[[i]] = as.list(c(predictions_cards_new[[i]], 
                                    predictions_cards[[old_forecaster_idxs[i]]]))
  }
}
predictions_cards = predictions_cards_comb
...
forecast dates to have the latest one by "2020-10-19" so that I wouldn't get
...
```{r}
scored_forecasters = lapply(score_cards, 
                            FUN = function(card) {
                              attributes(card[[1]])$name_of_forecaster
                              })
scored_forecasters = unlist(scored_forecasters)
...
# Create new score cards
score_cards_new = vector("list", length = length(forecasters))
for (i in 1:length(predictions_cards)) {
  name_of_forecaster = forecasters[i]
  cat(name_of_forecaster)
  given_dates = do.call("c", lapply(predictions_cards[[i]], function(x) {
    attributes(x)$forecast_date }))
  given_dates_trimmed = unique(given_dates)
  if (name_of_forecaster %in% scored_forecasters){
    forecaster_idx = which(scored_forecasters == name_of_forecaster)
    score_dates = score_cards[[forecaster_idx]][[1]]$forecast_date
    given_dates_trimmed = as_date(setdiff(given_dates_trimmed, score_dates))
  }
  ...
  is_evaluable_prediction = lapply(given_predictions, FUN = function(x) { 
    with(attributes(x), forecast_date < today() - 7 * ahead)})
  given_predictions = given_predictions[unlist(is_evaluable_prediction)]
  if(length(given_predictions) > 0){
    score_cards_new[[i]] = evaluate_predictions(given_predictions, err_measures,
                                                backfill_buffer = 0)
  } else {
    score_cards_new[[i]] = list()
  }
}

# And combine them with previous ones
score_cards_comb = list()
for (i in 1:length(forecasters)){
  name_of_forecaster = forecasters[i]
  if (name_of_forecaster %in% scored_forecasters){
    forecaster_idx = which(scored_forecasters == name_of_forecaster)
    forecaster_score_cards = list()
    if (length(score_cards_new[[i]]) > 0){
      for (weeks_ahead in ahead){
        forecaster_score_cards[[weeks_ahead]] = 
          rbind(score_cards[[forecaster_idx]][[weeks_ahead]], score_cards_new[[i]][[weeks_ahead]])
      }
      score_cards_comb[[i]] = forecaster_score_cards
    } else {
      score_cards_comb[[i]] = score_cards[[forecaster_idx]]
    }
  } else {
    score_cards_comb[[i]] = score_cards_new[[i]]
  }
}
score_cards = score_cards_comb

# May not be necessary, but better to keep it in sync.
for (i in 1:length(score_cards)){
  for (j in 1:length(score_cards[[i]])){
    attributes(score_cards[[i]][[j]])$forecast_dates = 
      sort(unique(score_cards[[i]][[j]]$forecast_date))
  }
}
```
...
```{r}
save(list = c("predictions_cards", "score_cards"), 
